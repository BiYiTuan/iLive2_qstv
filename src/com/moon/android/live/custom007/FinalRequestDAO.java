...
public class FinalRequestDAO {
		...
		if (NetworkUtil.isConnectingToInternet(MyApplication.getApplication())) {
			finalHttp.get(url, new AjaxCallBack<Object>() {
				public void onSuccess(Object t) {
					sendIntentMsg(t.toString(),
							UpgradeAndRequest.FIANL_DAO_UPDATE_MSG, handler);
				};

				public void onFailure(Throwable t, int errorNo, String strMsg) {
					sendIntentMsg("", UpgradeAndRequest.FIANL_DAO_UPDATE_MSG,
							handler);
				};
			});
		}
	...
	public void getProgramAd(final LiveData liveData, int requestTimes,
		...
		final List<String> list = new Configs().new RequestUrl().mListGetAd;
		...
		if (null != sf) {
		 ...
		 log.i("-------ddd--------:"+list.get(adPos) + tvid);
		if (NetworkUtil.isConnectingToInternet(ctx)) {
			 final String url = list.get(adPos) + tvid;
		
			 log.i("--------url----------"+url);
			finalHttp.get(url, new AjaxCallBack<Object>() {
				public void onSuccess(Object t) {
					String gsonStr = (String) t;
					log.i("Get tvid = " + liveData.getId() + " return "
							+ gsonStr);
					if (!gsonStr.equals("[]"))
		...
		if (NetworkUtil.isConnectingToInternet(MyApplication.getApplication())) {
			finalHttp.get(url, new AjaxCallBack<Object>() {
				public void onSuccess(Object t) {
					log.i("���������Ϣ�ɹ�");
					sendIntentMsg(t.toString(),
							UpgradeAndRequest.FIANL_DAO_APP_MSG);
				};
	...
	public void checkAuth(LiveActivity a, Handler h) {
		...
		mRequestUrl = new Configs().new RequestUrl();
		mCheckAuthCount = 0;
		FinalHttp finalHttp = new FinalHttp();
		...
		log.i("Auth------------------"+mRequestUrl.mListAuthAdress.get(mCheckAuthCount));
		if (NetworkUtil.isConnectingToInternet(a.getApplication())) {
			finalHttp.get(mRequestUrl.mListAuthAdress.get(mCheckAuthCount),
					mCheckAuthCallBack);
		}
	}
	...
	private AjaxCallBack<Object> mCheckAuthCallBack = new AjaxCallBack<Object>() {
		...
		public void onFailure(Throwable t, int errorNo, String strMsg) {
			...
			log.i("��ȨerrorNo " + errorNo + " strMsg " + strMsg);
			...
			finalHttp.get(
					mRequestUrl.mListAuthAdress.get(mCheckAuthCount % size),
					mCheckAuthCallBack);
			mCheckAuthCount++;
		}
	};
	...
	public void initPopWindow(final String appUrl, UpdateData updata,
			LiveActivity a, Handler h) {
		try {
			mActivity = a;
			mHandler = h;
			mUpdateUrl = appUrl;
			String updateMsg = updata.getMsg();
			log.i("updata.getType() = " + updata.getType());
			boolean isMustUpdate = "0".equals(updata.getType()) ? true : false;
			mUpdateMsg = null == updateMsg ? "" : updateMsg;
			View view = LayoutInflater.from(mActivity).inflate(
					R.layout.l_update_dialog, null);
			...
			int width = mActivity.getWindowManager().getDefaultDisplay()
					.getWidth();
	...
	private OnClickListener mDialogClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (v == mBtnSubmit) {
				Toast.makeText(mActivity, R.string.start_download,
	...
	public static void downFile(final String paramString,
			...
			public void run() {
				try {
					...
					if (localInputStream != null) {
						localFileOutputStream = context.openFileOutput(
								Configs.APK_NAME, 1);
	...
	public void showMsg(String paramString, LiveActivity a, Handler h) {
		...
		if ((paramString != null) && (!paramString.equals(""))) {
			...
			HorizontalMessageView localHorizontalMessageView = new HorizontalMessageView(
	...
	public void showExitWindow(LiveActivity a, Handler h) {
		...
		View view = mInflater.inflate(R.layout.l_p_exit_pop, null);

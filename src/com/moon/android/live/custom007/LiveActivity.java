...
public class LiveActivity extends Activity implements OnKeyListener {
	...
	private Map<String, List<LiveData>> mMapClassifyPrograms = new HashMap<String, List<LiveData>>();
	...
	private void initWidget() {
		mOptions = new DisplayImageOptions.Builder()
				.showImageOnLoading(R.drawable.l_icon_transport)
				.showImageForEmptyUri(R.drawable.l_icon_transport)
				.showImageOnFail(R.drawable.l_icon_transport)
		...
		mTextViewVersionName.setText(getText(R.string.version)
				+ VersionUtil.getVerName(this));
	}
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case Configs.NETWORK_CONNECT:
				...
				if (VodHistory.getAuthCacheStatus(LiveActivity.this)
						&& !StringUtil.isBlank(link)
				...
				// bundle.putString(Configs.INTENT_PARAM, vodUrl.getUrl());
				message.setData(bundle);
				Configs.key = "ZCM5P27nqPBuNQdI";
				Configs.link ="1sdf6rthbfghjkl8";
				sendMessage(message);
			...
			case Configs.AUTH_FAIL:
				if (loadPlayDataSucc) {
					...
					new CustomToast(getApplicationContext(),
							getString(R.string.exit_5s_later)).show();
			...
			case Configs.READ_CACHE:
				/***
				 * 2014.7.22 ��ѡ��ȡ���ػ�����б?�����ػ�����б��У���ֱ��ʹ�ñ��ص��б�
				 * ͬʱ��ȡ�����б?ȡ���Ժ�����и��£����滻���ػ��� ͬʱ�����б�
				 */
				mTextLoadInMsg.setText(R.string.load_list);
				if (ProgramCache.isExist(LiveActivity.this)
						&& !ProgramCache.isDirectory(LiveActivity.this)) {
					// �����б�
					log.i("��ȡ�����е��б�");
					String gsonCacheStr = ProgramCache
							.getGsonString(LiveActivity.this);
					// ��ȡ�����е��б?�����е��б�������?������ȥ�б?ȥ���б������
					// ��������б�δ���?��ȡ��ݣ�ȡ����ݺ���жԱ�
					// MD5ֵ����ͬ�����±��ػ�����ݣ�Ȼ��ˢ���б�
					try {
			...
			case Configs.AUTH_SUCCESS_CACHE:
				...
				subcatUrl2 = Configs.SERVER + "/Api/tvjson?mac="
						+ MACUtils.getMac();
				log.i(subcatUrl2);
				...
				if (NetworkUtil.isConnectingToInternet(LiveActivity.this)) {
					...
					finalHttp.get(subcatUrl2, mParseVodCallBack);
				}
				break;
			...
			case Configs.LOAD_DATA_SUCCESS:
				...
				getLastLookAndPlay();
				mOsdListProgram.setProgramId(String
						.valueOf(mCurrentPlayPos + 1));
				try {
					mTimerCache.schedule(mTimerTaskTraffic,
							START_CACHE_TIME * 1000, GET_CACHE_PEROID * 1000);
				} catch (Exception e) {
					log.e(e.toString());
				}
				break;
			...
			case IS_FIRST_VOD_OK:
				...
				if (mCurrentPlayPos + 1 >= mListLiveDataAll.size()
						|| mCurrentPlayPos + 1 <= 0) {
			...
			case PALY_HTTP_VOD:
				...
				mVideoPath = bundle3.getString(Configs.INTENT_PARAM);
				startPlay();
				break;
		...
		private void startPlay() {
			...
			if (!loadPlayDataSucc) {
				...
				mTimerFisrtCanPlay.schedule(mTimerTaskFirstCanPlay,
						FIRST_VOD_READY_TIME);
			}
		}
	};
		...
		public void handleMessage(Message msg) {
			...
			if (traffic <= 0) {
				...
				if (!checkNetworkConnected()) {
					...
					mOsdManager.closeOSD(mOsdNetwork);
			} else
				mOsdManager.closeOSD(mOsdNetwork);
		};
	};
	...
	private void update() {
		...
		intent.setDataAndType(Uri.fromFile(new File(Configs.APP_DATA_PATH
				+ "/files/", Configs.APK_NAME)),
	...
	private BroadcastReceiver mMsgReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Configs.BroadCast.UPDATE_MSG.equals(intent.getAction())) {
				...
				new FinalRequestDAO().initPopWindow(appUrl, updata,
						LiveActivity.this, mHandler);
			}
		}
	};
		...
		public void onClick(View v) {
			...
			if (loadPlayDataSucc)
				if (mOsdListProgram.getVisibility() == View.GONE)
					...
					mOsdManager.closeOSD(mOsdListProgram);
		}
	};
	...
	private AjaxCallBack<Object> mParseVodCallBack = new AjaxCallBack<Object>() {
		...
		public void onSuccess(Object t) {
			...
			log.i(gsonStr);
			if (loadPlayDataSucc) {
				if (!ProgramCache.checkIsSame(gsonStr, LiveActivity.this)) {
					log.i("��ȡ���б����ݿ��е��б?ͬ������Ƿ����");
					try {
						// ���Գɹ������ٽ���ݸ���
						Gson gson = new Gson();
						gson.fromJson(gsonStr, new TypeToken<List<LiveData>>() {
						}.getType());
						new CustomToast(getApplicationContext(),
								getString(R.string.data_is_update)).show();
						log.i("���µ������������������");
	...
	private void parseXmlAndPlay(String gsonString, boolean isLoadCache,
			String key) {
	...
	private boolean parseListXML(String gsonString, boolean isLoadCache,
			String key) {
		Gson gson = new Gson();
		List<LiveData> listLiveData = null;
		try {
			listLiveData = (List<LiveData>) gson.fromJson(gsonString,
					new TypeToken<List<LiveData>>() {
					}.getType());
		} catch (Exception e) {
			log.e(e.toString());
			return false;
		...
		for (LiveData data : listLiveData) {
			...
			try {
				...
				if (urlDecode.startsWith("force://")
					...
					String[] arrayOfString = urlDecode.split("://")[1]
							.split("/");
				...
				// ����Ŀ�Ƿ��Ѿ�����
				boolean isExist = mMapCheckProgramRepeat.containsKey(data
						.getId());
				if (isExist) {
					data.setInAllProgramPos(mMapCheckProgramRepeat.get(data
							.getId()));
				} else {
					data.setInAllProgramPos(String.valueOf(programPostion));
					mMapCheckProgramRepeat.put(data.getId(),
							String.valueOf(programPostion));
					programPostion++;
					listAll.add(data);
				}
		...
		mMapClassifyPrograms.put("0|" + getString(R.string.program_title),
				mListLiveDataAll);
		return true;
	}
	...
	private void putProgramToClassify(LiveData data) {
		...
		if (null == classifyName || "".equals(classifyName) || null == orderid
				|| "".equals(orderid))
			return;
	...
	private void initOSD() {
		...
		mOsdListProgram = new OSDListProgram(getApplicationContext(), this,
				mContainer, mMapClassifyPrograms);
	...
	private void play(LiveData liveData) {
		...
		if (null != liveData && null != url) {
			if (url.startsWith("forcetv://") || url.startsWith("force://")) {
				FinalHttp finalHttp = new FinalHttp();
				String requestUrl = getVideoHttp(liveData.getChannelId(),
						liveData.getStreamIp(), FORCE_CMD, "ZCM5P27nqPBuNQdI");
				log.i("[" + requestUrl + "]" + "link:" + Configs.link);
	...
	private void prarseAdAndShow(int position) {
		try {
			...
			log.i("--------444:" + liveData.getId());
			if (!liveData.isHasGetAd()){
				/*new FinalRequestDAO().getProgramAd(liveData, 0,
						MainActivity.this);*/
				FinalRequestDAO fr =new FinalRequestDAO();
				fr.getProgramAd(liveData,0,LiveActivity.this);
			}
				

			log.i("--------555:" + liveData.getId());
		} catch (Exception e) {
			log.e("111111111111");
			log.e(e.toString());
		}
	...
	private AjaxCallBack<Object> mStartPlayCallBack = new AjaxCallBack<Object>() {
		...
		public void onSuccess(Object t) {
			...
			if (null == forceLoadItem || forceLoadItem.getFlag() != 0) {
				log.i("program start fail, reson = "
						+ forceLoadItem.getReason());
				mOsdManager.closeOSD(mOsdCache);
				if (!loadPlayDataSucc) {
					toPlay();
				}
		...
		private void toPlay() {
			...
			mVideoPath = new StringBuilder(PLAY_SERVER)
					.append(liveData.getChannelId()).append(PERIOD)
					.append(liveData.getType()).toString();
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		...
		if (event.getAction() == KeyEvent.ACTION_DOWN && v == mVideoView
				&& loadPlayDataSucc) {
			switch (keyCode) {
			...
			if (keyCode >= KeyEvent.KEYCODE_0 && keyCode <= KeyEvent.KEYCODE_9) {
				if (!isInputProgramNum) {
					...
					mOsdProgramNum.setVisibility(View.VISIBLE);
					String oldText = String.valueOf(mOsdProgramNum
							.getProgramText());
					if (oldText.length() >= 3) {
						oldText = "";
					}
					...
					mOsdProgramNum
							.setProgramText(
									oldText + newText,
									getProgramName(Integer.valueOf(oldText
											+ newText) - 1));
					reTimingTVNumInput();
				}
			}
			...
			if (keyCode == KeyEvent.KEYCODE_DEL && isInputProgramNum) {
				String oldText = String
						.valueOf(mOsdProgramNum.getProgramText());
				int oldTextLength = oldText.length();
				if (oldTextLength >= 1) {
					String newText = oldText.substring(0, oldTextLength - 1);
					if (null == newText
							|| StringUtil.NULL_STRING.equals(newText)) {
						cancelTVNumInputTimer();
					} else
						mOsdProgramNum.setProgramText(newText,
								getProgramName(Integer.valueOf(newText)));
					reTimingTVNumInput();
				}
			}
		}
		...
		public void handleMessage(android.os.Message msg) {
			...
			case START_CHANGE_CHANNEL:
				...
				if (destProgramId != mCurrentPlayPos) {
					if (destProgramId >= 0
							&& destProgramId < mListLiveDataAll.size()) {
						...
						mOsdProgramNum.setProgramText(
								String.valueOf(destProgramId + 1),
								getString(R.string.no_program));
					isInputProgramNum = false;
				} else {
					mOsdProgramNum.setProgramText(
							String.valueOf(destProgramId + 1),
							getString(R.string.is_playing));
				}
				break;
			}
		};
	...
	private void showProgramNum(int inAllPos) {
		int programPos = inAllPos <= 0 || inAllPos >= mListLiveDataAll.size() ? 0
				: inAllPos;
		mOsdProgramNum.setProgramText(String.valueOf(inAllPos + 1),
				mListLiveDataAll.get(programPos).getDname());
		mOsdProgramNum.setVisibility(View.VISIBLE);
	}
		...
		public void onSuccess(JSONObject response) {
			...
			try {
				...
				if (0 == error_code) {
					...
					WhiteListControl.setHeartBeat(LiveActivity.this,
							MACUtils.getMac(), serverAddress, serverPort);
	...
	public void getHeartChip() {
		Pull p = new Pull();
		try {
			URL ur = new URL(new Configs().HBS);
	...
	private void loginWhiteServer() {
		String deviceID = MACUtils.getMac();
		if ((deviceID != null) && !(deviceID.equals(""))) {
			log.d("login whiteList Server!");
			log.d("deviceID=" + deviceID);
			log.d("loginURL=" + Configs.WHITE_URL);
			WhiteListControl.sendLogin(deviceID, Configs.WHITE_URL,
					loginHandler);
		}
	}
}
